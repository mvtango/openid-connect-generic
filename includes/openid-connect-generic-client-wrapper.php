...
class OpenID_Connect_Generic_Client_Wrapper {
	...
	function authentication_request_callback() {
		...
		// error_log('TOKEN RESULT' . print_r($token_result,TRUE));
			...
			foreach ( $matches[ 0 ] as $match ) {
				...
				if ( ! isset( $user_claim[ $key ] ) ) {
					if ( $error_on_missing_key ) {
						return new WP_Error( 'incomplete-user-claim', __( 'User claim incomplete - key "' . $key . '" required by "' . $format . '" is missing ' ), $user_claim );
					}
				} else {
