...
class OpenID_Connect_Generic_Client {
	...
	private $state_time_limit = 180000;
	...
	function validate_authentication_request( $request ){
		...
		if ( ! isset( $request['state'] ) ) {
			return new WP_Error( 'missing-state', __( 'Missing state while validating authentication request.' ), $request ); 
		}

		if ( isset( $request['state'] ) && ! $this->check_state( $request['state'] ) ){
			return new WP_Error( 'checking-state', __( 'Check State failed. ' . $request['state'] ), $request ); 
		}
		return $request;
	}
	...
	function validate_token_response( $token_response ){
		...
		if ( ! isset( $token_response['id_token'] ) ) {
			return new WP_Error( 'invalid-token-response-idtoken', 'ID Token missing', $token_response );
		}
		if ( ! isset( $token_response['access_token'] ) )
		{
			return new WP_Error( 'invalid-token-response-accesstoken', 'Access Token missing', $token_response );
		}
		if ( ! isset( $token_response['token_type'] ) || strcasecmp( $token_response['token_type'], 'Bearer' ))
		   {
			return new WP_Error( 'invalid-token-response-type', 'Token type: bearer missing', $token_response );
	...
	function get_user_claim( $token_response ){
		...
		$user_claim = json_decode( $user_claim_result['body'], TRUE );
	        error_log("User Claim:" . print_r($user_claim,TRUE));
		return $user_claim;
	}
